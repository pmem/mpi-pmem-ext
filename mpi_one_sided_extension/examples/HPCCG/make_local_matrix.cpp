...
void make_local_matrix(HPC_Sparse_Matrix * A) {
   ...
   ///////////////////////////////////////////////////////////////////////////
   // Build "send_offset" list. Each entry specifies offset in dest neighbor.
   ///////////////////////////////////////////////////////////////////////////

   MPI_MY_TAG++;

   int* send_offset = new int[num_recv_neighbors];
   A->send_offset = send_offset;

   for (i = 0; i < num_recv_neighbors; i++) {
      MPI_Irecv(send_offset + i, 1, MPI_INT, neighbors[i],
                MPI_MY_TAG, MPI_COMM_WORLD, request + i);
   }

   int offset = local_nrow;
   for (i = 0; i < num_recv_neighbors; i++) {
      MPI_Send(&offset, 1, MPI_INT, neighbors[i], MPI_MY_TAG, MPI_COMM_WORLD);
